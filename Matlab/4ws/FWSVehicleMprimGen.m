...
output_filename =               'xa_4ws_crab_littleAngles_ZeroCost';
car_width =                     2.7;
car_length_back =               1.3;
car_length_front =              5.3;
distance_between_axes =         3.2;
max_steering_radians =          0.3;
numberofangles =                8;
steeringanglepartitions =       1;
steeringanglecardinality =      1;
distance =                      15;
...
% car_width =                     0.6;
% car_length_back =               1.2;
% car_length_front =              1.2;
% distance_between_axes =         2.0;
% 
...
% Cost multipliers
forwardcostmult = 0; %1.0;
forwardturncostmult =0; % 1.2;
crabcostmult = 0; %1.1;
parkingcostmult = 0; %2.0;
backwardcostmult = 0; %1.0;
backwardturncostmult =0; % 1.2;
...
%                                 %4WS MODE
%                                     [intermcells_m, motion] = BiwsVehicleSolver(distance_between_axes, ...
%                                     [start_x, start_y, start_o, start_phi,start_phi], ...
%                                     [goal_x, goal_y, goal_o, goal_phi, goal_phi], ...
%                                         0, 0);
%                                 
    ...
    for steerind = 1:steeringanglecardinality
        ...
        for primind = 1:size(all_base_primitives{angleind,steerind},2)
            ...
            % prune primitives which are too long
             prLengGain = 1;
                if (all_base_primitives_motions{angleind,steerind}{primind} == 2 || ...
                    all_base_primitives_motions{angleind,steerind}{primind} == 3)

                    prLengGain = CrabDistGain;
                end
            ...
            if valid_primitive
                ...
                if primitive_length > sqrt((goal_x - start_x)^2 + (goal_y - start_y)^2) * prLengGain*allowed_primitive_length_multiplier
                    valid_primitive = false;
                end
            end
...
for angleind = ((numberofangles/8)+2):numberofangles/4+1
    for steerind = 1:steeringanglecardinality
        ...
        for primind = 1:size(primitives{numberofangles/4 + 2 - angleind, steeringanglecardinality - steerind + 1},2)
            ...
            primitives{angleind,steerind}{primind}((logical(primitives{angleind,steerind}{primind}(:,5) - approxError < -pi)), 5) = pi;
...
for angleind = ((numberofangles/4)+2):numberofangles/2+1
    for steerind = 1:steeringanglecardinality
        ...
        for primind = 1:size(primitives{numberofangles/2 + 2 - angleind, steeringanglecardinality - steerind + 1},2)
            ...
            primitives{angleind,steerind}{primind}(logical(primitives{angleind,steerind}{primind}(:,3) - approxError < -pi), 3) = pi;
            primitives{angleind,steerind}{primind}(logical(primitives{angleind,steerind}{primind}(:,4) - approxError < -pi), 4) = pi;
            primitives{angleind,steerind}{primind}(logical(primitives{angleind,steerind}{primind}(:,5) - approxError < -pi), 4) = pi;
...
%             end
            ...
            for interind = 1:size(intermcells_m, 1)
                fprintf(fout, '%.4f %.4f %.4f %.4f %.4f\n', intermcells_m(interind,1), intermcells_m(interind,2), intermcells_m(interind,3), intermcells_m(interind,4),intermcells_m(interind,5));
            end
