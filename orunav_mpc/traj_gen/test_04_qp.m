...
load ../test/test_04_data/trajectory.txt
load ../test/test_04_data/sensors.txt
outfile = '../test/test_04_data/solution.txt';
...
sensors = [(sensors(:,1:3) - sensors(1,1:3)) sensors(:,4)]';
sensors(:,3) = atan2(sin(sensors(:,3)), cos(sensors(:,3)));
...
for k = 2:size(state, 2) - Np + 1
    ...
    [X, FVAL, OUTPUT, LAMBDA] = qp ([], P, p, [], [], lb, ub, phi_g_l, phi_GT, phi_g_u);
