...
namespace orunav_generic
{
  ...
  std::vector<int> loadIntVecTextFile(const std::string &fileName) {
  ...
  void saveIntVecTextFile(const std::vector<int> &vec, const std::string &fileName)
 {
   std::stringstream st;
   st << fileName;
   std::string file_name = st.str();
...
Path loadPathTextFile(const std::string &fileName)
{
...
std::vector<orunav_generic::Path> loadPathDirTextFile(const std::string &dirName)
{
 ...
 void savePathTextFile(const PathInterface &path, const std::string &fileName)
 {
   std::stringstream st;
 ...
 void saveQuaternionPathTextFile(const PathInterface &path, const std::string &fileName) 
 { 
   std::stringstream st;
   st << fileName;
   std::string file_name = st.str();
...
Trajectory loadTrajectoryTextFile(const std::string &fileName)
 {
 ...
 void saveTrajectoryTextFile(const TrajectoryInterface &traj, const std::string &fileName)
 {
   std::stringstream st;
   st << fileName; // << "_" << msg.robot_id << "-" << msg.goal_id << ".path";
   std::string file_name = st.str();
...
void saveTrajectoryChunksTextFile(const TrajectoryChunksInterface &chunks, const std::string &fileName) {
 std::stringstream st;
   st << fileName; // << "_" << msg.robot_id << "-" << msg.goal_id << ".path";
   std::string file_name = st.str();
   std::ofstream ofs(file_name.c_str());
 ...
 void saveTraversedDistanceTrajectoryTextFile(const TrajectoryInterface &traj, const std::string &fileName)
 {
   std::stringstream st;
   st << fileName; // << "_" << msg.robot_id << "-" << msg.goal_id << ".path";
   std::string file_name = st.str();
 ...
 CoordinatedTimes loadCoordinatedTimesTextFile(const std::string &fileName) {
   std::vector<double> tmp = orunav_generic::loadDoubleVecTextFile(fileName);
   return CoordinatedTimes(tmp);
 }

void saveDeltaTInterface(const DeltaTInterface &dts, const std::string &fileName) {
  saveDoubleVecTextFile(getDoubleTimeVecFromDeltaTInterface(dts), fileName);
}

 
// Gnuplot related functions
std::string getPoint2dContainerInterfaceGnuplotString(const Point2dContainerInterface &pts, bool duplicateFirstPointLast) {
